<%-- 
    Document   : profile
    Created on : 1 Jan 2026, 4:16:20 am
    Author     : SHAHRUL
--%>
...
<%
  ...
  if (s == null) {
    response.sendRedirect(request.getContextPath() + "/login"); // mapping later
    return;
  }

  Student data = (Student) request.getAttribute("studentData");
  if (data == null) data = s;

  String currentFaculty = (data.getFaculty() == null) ? "" : data.getFaculty();
...
<head>
  ...
  <style>
    ...
    /* ── Page Card ── */
    .title-row{display:flex;align-items:flex-end;justify-content:space-between;gap:10px;flex-wrap:wrap;margin-bottom:14px}
    .title-row h2{margin:0;font-weight:950;color:#111827}
    .muted{color:#6b7280}

    /* ── Form Controls ── */
    .form-label{font-weight:900;color:#374151;font-size:.9rem}
    .form-control,.form-select{border-radius:16px;background:#f4f6f8;border:1px solid transparent;padding:12px 14px;transition:.2s}
    .form-control:focus,.form-select:focus{background:#fff;border-color:#efe6ff;box-shadow:0 0 0 .25rem rgba(111,60,255,.15)}
    .readonly{background:#f8f9fb !important;color:#6b7280}

    /* ── Buttons (Bootstrap override) ── */
    .btn-soft{border-radius:16px;padding:10px 14px;font-weight:900;border:1px solid #efe6ff;background:#fff;color:#6f3cff}
    .btn-soft:hover{background:#faf8ff}
    .btn-purple{background:#6f3cff;border:0;color:#fff;border-radius:16px;padding:12px 18px;font-weight:950;box-shadow:0 18px 40px rgba(111,60,255,.18)}
    .btn-purple:hover{background:#5f2df0}

    /* ── Bell/dot (profile page uses bell in header) ── */
    .bell{width:40px;height:40px;border-radius:12px;border:0;background:transparent;display:flex;align-items:center;justify-content:center;color:#9ca3af;position:relative}
    .bell:hover{background:#f3f4f6;color:#6f3cff}
    .dot{width:8px;height:8px;border-radius:999px;background:#ef4444;position:absolute;top:10px;right:10px}
  </style>
</head>
...
<body>
  ...
  <div class="app">
    ...
    <nav class="menu">
      ...
      <a class="active" href="<%=request.getContextPath()%>/student/profile">
    ...
    <main class="main">
      ...
      <header class="header">
        ...
        <div class="header-right">
          ...
          <div class="userbox">
            <div class="usertext">
              <div style="font-weight:950;color:#111827;font-size:14px"><%= s.getStudentName() %></div>
              <div style="font-size:11px;color:#6b7280;font-weight:950;letter-spacing:.10em;text-transform:uppercase">
                <%= s.getMatricNo() %>
              </div>
            </div>
      ...
      <div class="content">
        ...
        <div class="title-row">
          ...
          <a class="btn btn-soft" href="<%=request.getContextPath()%>/student/dashboard">Back</a>
        </div>
        ...
        <% } %>
        <% if (ok != null) { %>
          <div class="alert alert-success rounded-4"><%= ok %></div>
        <% } %>
          ...
          <form method="post" action="<%=request.getContextPath()%>/student/profile">
            ...
            <div class="row">
              <div class="col-md-6 mb-3">
                ...
                <input class="form-control readonly" value="<%= data.getStudentId() %>" disabled>
              </div>
              <div class="col-md-6 mb-3">
                <label class="form-label">Matric No</label>
                <input class="form-control readonly" value="<%= data.getMatricNo() %>" disabled>
              </div>
            </div>

            <div class="mb-3">
              <label class="form-label">Full Name</label>
              <input name="name" class="form-control" value="<%= data.getStudentName() %>" required>
            </div>

            <div class="row">
              <div class="col-md-6 mb-3">
                <label class="form-label">Email</label>
                <input class="form-control readonly" value="<%= data.getEmail() %>" disabled>
              </div>
              <div class="col-md-6 mb-3">
                <label class="form-label">Phone Number</label>
                <input name="phone" class="form-control" value="<%= data.getPhoneNumber() %>" required>
              </div>
            </div>
            ...
            <div class="mb-4">
              ...
              <select name="faculty" class="form-select" required>
                ...
                <option value="FSKM"  <%= "FSKM".equals(currentFaculty) ?  "selected" : "" %>>Faculty of Science Computer And Mathematics (FSKM)</option>
                <option value="FTKK"  <%= "FTKK".equals(currentFaculty) ?  "selected" : "" %>>Faculty of Ocean Engineering Technology (FTKK)</option>
                <option value="FSSM"  <%= "FSSM".equals(currentFaculty) ?  "selected" : "" %>>Faculty of Science and Marine Environment (FSSM)</option>
                <option value="FSPA"  <%= "FSPA".equals(currentFaculty) ?  "selected" : "" %>>Faculty of Fisheries and Aquaculture Science (FSPA)</option>
                <option value="FSMA"  <%= "FSMA".equals(currentFaculty) ?  "selected" : "" %>>Faculty of Food Science and Agrotechnology (FSMA)</option>
                <option value="FPEPS" <%= "FPEPS".equals(currentFaculty) ? "selected" : "" %>>Faculty of Business, Economics and Social Development (FPEPS)</option>
                <option value="FPM"   <%= "FPM".equals(currentFaculty) ?   "selected" : "" %>>Faculty of Maritime Studies (FPM)</option>
                <option value="PASTEM"   <%= "PASTEM".equals(currentFaculty) ?   "selected" : "" %>>Foundation of STEM (PASTEM)</option>
              </select>
            </div>
