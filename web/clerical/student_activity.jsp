<%-- 
    Document   : student_activity.jsp (Clerical)
    Author     : SHAHRUL
--%>
...
<%
    ...
    List<Map<String, Object>> studentList =
        (List<Map<String, Object>>) request.getAttribute("studentList");
...
<head>
  <meta charset="UTF-8">
  <title>Student Activity | Smart Campus</title>
  ...
  <style>
    ...
    .student-card{background:#fff;border:1px solid #e5e7eb;border-radius:20px;padding:20px;box-shadow:0 4px 12px rgba(17,24,39,.04);transition:.15s;cursor:pointer;text-decoration:none;display:block;color:inherit}
    .student-card:hover{border-color:#6f3cff;box-shadow:0 8px 24px rgba(111,60,255,.1);transform:translateY(-2px);color:inherit}
    .student-avatar{width:48px;height:48px;border-radius:999px;background:#ede9fe;color:#6f3cff;display:flex;align-items:center;justify-content:center;font-weight:950;font-size:18px;flex-shrink:0}
    .student-info{flex:1;min-width:0}
    .student-name{font-weight:950;color:#111827;font-size:14px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
    .student-meta{font-size:12px;color:#6b7280;font-weight:600;margin-top:2px}

    /* ── Mini Stats inside Card ── */
    .stats-mini{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-top:14px;padding-top:14px;border-top:1px solid #e4d9f7}
    .stat-mini{text-align:center}
    .stat-mini-value{font-size:18px;font-weight:950;color:#111827}
    .stat-mini-label{font-size:10px;font-weight:900;color:#9ca3af;text-transform:uppercase;letter-spacing:.08em}
    .stat-mini.unpaid .stat-mini-value{color:#ea580c}
    .stat-mini.paid .stat-mini-value{color:#16a34a}
    .stat-mini.total .stat-mini-value{color:#6f3cff}

    /* ── Outstanding Tag ── */
    .outstanding-tag{font-size:11px;font-weight:950;padding:4px 10px;border-radius:999px;background:#fff7ed;color:#ea580c;border:1px solid #ffedd5;display:inline-block;margin-top:8px}
    .clear-tag{font-size:11px;font-weight:950;padding:4px 10px;border-radius:999px;background:#f0fdf4;color:#16a34a;border:1px solid #dcfce7;display:inline-block;margin-top:8px}
  </style>
</head>
<body>
    ...
    <div class="content">
      ...
      <div class="page-header">
        <div>
          ...
          <p>View all registered students and their summons records.</p>
        </div>
      </div>

      <!-- Filter Bar -->
      <div class="filter-bar">
        <input type="text" id="searchInput" placeholder="Search by name, matric or faculty..."
               oninput="filterCards()" style="flex:1;min-width:200px"/>
        <select id="filterStatus" onchange="filterCards()">
          <option value="">All Students</option>
          <option value="has-unpaid">Has Unpaid Summons</option>
          <option value="clean">No Summons</option>
        </select>
        <select id="filterFaculty" onchange="filterCards()">
          <option value="">All Faculties</option>
          <option value="FSKM">FSKM</option>
          <option value="FTKK">FTKK</option>
          <option value="FSSM">FSSM</option>
          <option value="FSPA">FSPA</option>
          <option value="FSMA">FSMA</option>
          <option value="FPEPS">FPEPS</option>
          <option value="FPM">FPM</option>
          <option value="PASTEM">PASTEM</option>
        </select>
      </div>
      ...
      <div class="students-grid" id="studentsGrid">
        <% for (Map<String, Object> st : studentList) {
             String name       = (String) st.get("studentName");
             String studentId  = (String) st.get("studentId");
             String matricNo   = (String) st.get("matricNo");
             String faculty    = (String) st.get("faculty");
             String email      = (String) st.get("email");
             int    total      = (int)    st.get("totalSummons");
             int    unpaid     = (int)    st.get("unpaid");
             int    paid       = (int)    st.get("paid");
             double outstanding= (double) st.get("outstanding");
             int    vehicles   = (int)    st.get("vehicleCount");
             String initial    = name != null && name.length() > 0 ? name.substring(0,1).toUpperCase() : "?";
        %>
        <a href="<%=request.getContextPath()%>/clerical/students/view?id=<%= studentId %>"
           class="student-card"
           data-name="<%= name.toLowerCase() %>"
           data-matric="<%= matricNo.toLowerCase() %>"
          ...
          <div style="display:flex;gap:14px;align-items:center">
            ...
            <div class="student-info">
              ...
              <div class="student-meta"><%= matricNo %> • <%= faculty %></div>
          ...
          <div class="stats-mini">
            ...
            <div class="stat-mini unpaid">
              <div class="stat-mini-value"><%= unpaid %></div>
              <div class="stat-mini-label">Unpaid</div>
            </div>
            <div class="stat-mini paid">
              <div class="stat-mini-value"><%= paid %></div>
          ...
          <div style="display:flex;justify-content:space-between;align-items:center;margin-top:10px">
            <% if (outstanding > 0) { %>
            <span class="outstanding-tag">RM <%= String.format("%.2f", outstanding) %> owed</span>
            <% } else if (total == 0) { %>
            <span class="clear-tag">✓ No Summons</span>
            <% } else { %>
            <span class="clear-tag">✓ All Clear</span>
            <% } %>
...
<script>
  function filterCards() {
    ...
    cards.forEach(card => {
      ...
      const fac     = card.getAttribute("data-faculty") || "";
      const unpaid  = parseInt(card.getAttribute("data-unpaid")) || 0;
      const total   = parseInt(card.getAttribute("data-total")) || 0;

      const matchSearch  = name.includes(search) || matric.includes(search);
      const matchFaculty = !faculty || fac === faculty;
      let   matchStatus  = true;
      if (status === "has-unpaid") matchStatus = unpaid > 0;
      if (status === "clean")      matchStatus = total  === 0;

      card.style.display = (matchSearch && matchFaculty && matchStatus) ? "" : "none";
    });
  }
